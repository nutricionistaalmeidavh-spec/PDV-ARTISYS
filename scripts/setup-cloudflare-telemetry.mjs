...
function parseD1List(stdout,databaseName){let rows;try{rows=JSON.parse(String(stdout||'[]'));}catch{throw new Error('Nao foi possivel interpretar `wrangler d1 list --json`.');}const matches=(Array.isArray(rows)?rows:[]).filter(item=>item?.name===databaseName);if(matches.length>1)throw new Error(`Encontrado mais de um D1 chamado ${databaseName}; descoberta ambigua.`);return matches[0]||null;}
...
async function provisionTelemetry({cwd=process.cwd(),configPath=path.join(cwd,'cloudflare','telemetry','wrangler.jsonc'),databaseName='artisys-telemetry',runner=defaultRunner,fetchImpl=globalThis.fetch,log=console.log}={}){
  ...
  const run=(args)=>runner('npx',args,{cwd});
  ...
  const config=readConfig(configPath);const analytics=(config.analytics_engine_datasets||[]).find(item=>item.binding==='ANALYTICS');if(!analytics?.dataset)throw new Error('Binding ANALYTICS nao configurado em wrangler.jsonc.');
  log('Aplicando migrations D1 remotas...');await run(['wrangler','d1','migrations','apply',databaseName,'--remote','--config',configPath]);
  ...
  const health=await fetchImpl(`${workerUrl}/health`,{headers:{accept:'application/json'}});if(!health?.ok)throw new Error(`Health check do Worker falhou com HTTP ${health?.status||0}.`);
