...
async function receiptApiRequest(rawPath,sessionToken,{method='GET',body=undefined}={}) {
  ...
  const response=await fetch(`${apiBase}${rawPath}`,{method,headers,body:encodedBody});
...
async function createSalePrintAttempt(saleId, sessionToken) {
  return receiptApiRequest(`/api/v1/sales/${encodeURIComponent(saleId)}/print-attempts`,sessionToken,{method:'POST'});
}

async function finishSalePrintAttempt(saleId, jobId, outcome, sessionToken) {
  return receiptApiRequest(`/api/v1/sales/${encodeURIComponent(saleId)}/print-attempts/${encodeURIComponent(jobId)}/result`,sessionToken,{method:'POST',body:outcome});
}
