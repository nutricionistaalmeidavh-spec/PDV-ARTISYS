...
(() => {
  ...
  const roleAllowed = () => ['admin','manager'].includes(String(state.session?.user?.role || ''));
  ...
  function refreshDraftTotal() {
    ...
    if (submit) submit.disabled = !roleAllowed() || total <= 0;
  }
  ...
  function renderSaleDetails() {
    ...
    const itemRows = (state.sale.items || []).map(item => {
      ...
      return `<div class="return-item-row" data-return-item="${escapeHtml(item.id)}">
        <label class="return-item-check"><input type="checkbox" data-return-check ${available <= 0 || !allowed ? 'disabled' : ''}><span><strong>${escapeHtml(item.productName || 'Item')}</strong><small>Vendido ${Number(item.quantity || 0).toLocaleString('pt-BR',{maximumFractionDigits:3})} · já devolvido ${Number(already).toLocaleString('pt-BR',{maximumFractionDigits:3})}</small></span></label>
        <label><small>Quantidade</small><input data-return-qty type="number" min="0.001" step="0.001" max="${available}" value="${available > 0 ? Math.min(1,available) : 0}" ${available <= 0 || !allowed ? 'disabled' : ''}></label>
    ...
    root.innerHTML = `
      ...
      ${allowed ? '' : '<div class="return-permission-warning">A devolução exige sessão de gerente ou administrador. A consulta permanece disponível.</div>'}
      <section class="return-items"><h3>Itens e quantidades</h3>${itemRows}</section>
      <section class="return-refund-grid">
        <label class="field wide"><span>Motivo *</span><textarea id="return-reason" rows="3" placeholder="Ex.: produto devolvido pelo cliente" ${allowed ? '' : 'disabled'}></textarea></label>
        <label class="field"><span>Forma de reembolso</span><select id="return-refund-method" ${allowed ? '' : 'disabled'}>${VALID_REFUNDS.map(method => `<option value="${method}" ${method === originalMethod ? 'selected' : ''}>${refundLabel(method)}</option>`).join('')}</select></label>
  ...
  async function submitReturn() {
    ...
    if (!roleAllowed()) {
      setStatus('A devolução exige gerente ou administrador.','error');
      return;
    }
