...
(() => {
  ...
  function productCard(product) {
    return `<button type="button" class="product-card" data-add-product="${product.id}"><div><div class="product-visual"><span>${escapeHtml(initials(product.name))}</span></div><h3>${escapeHtml(product.name)}</h3><small>Cód. ${escapeHtml(product.sku || product.barcode || product.id.slice(0, 8))}</small></div><strong>${ui.formatCents(product.salePriceCents)}<span class="add-cart">＋</span></strong></button>`;
  }
  ...
  async function cancelCurrentSale() {
    ...
    openModal('Cancelar venda', `<p>Informe o motivo do cancelamento da venda atual.</p><div class="field"><label>Motivo</label><input id="cancel-reason" value="Cliente desistiu"></div><div class="modal-actions"><button class="secondary-button" data-close-modal>Voltar</button><button class="danger-button" id="confirm-cancel">Cancelar venda</button></div>`, { onMount(root) { root.querySelector('[data-close-modal]')?.addEventListener('click', closeModal); root.querySelector('#confirm-cancel')?.addEventListener('click', async () => { try { await api.cancelSale(state.sale.id, root.querySelector('#cancel-reason').value); state.sale = null; state.selectedProductId = null; closeModal(); renderCheckout(); showToast('Venda cancelada.', 'success'); } catch (error) { showToast(error.message, 'error'); } }); } });
  }
