...
async function defaultExecuteFlow({manifest,rootDir,environmentName,environment,viewport,outputRoot,flowId}){
  const {file:flowFile}=resolveFlow(manifest,flowId,rootDir);
  return runQaFlow({manifest,rootDir,environmentName,environment,flowName:flowId,flowFile,viewport,outputRoot});
}

async function readTelemetry(outputDir){
  if(!outputDir)return[];
...
async function loadModuleCoverage({manifest,rootDir}){
  ...
  const checks=plan.contracts.map(contract=>{
    ...
    return{
      ...
      status:covered?'passed':'not-applicable',
      critical:contract.critical,
      details:{moduleId:contract.moduleId,launcher:contract.launcher,protected:contract.protected,liveSync:contract.liveSync,dependsOn:contract.dependsOn},
    };
  });
...
export async function generateCrosscutProfile({
  ...
  executeFlow=defaultExecuteFlow,
  ...
  for(const flowId of flowIds){
    ...
    try{
      const result=await executeFlow({manifest,rootDir,environmentName,environment,viewport,outputRoot,flowId,flow:manifest.flows[flowId]});
      runs.push({flowId,status:'passed',...result});
      checks.push({name:flowId,category:categories[flowId]||'crosscut-flow',status:'passed',critical:criticalFlows.has(flowId),durationMs:result?.durationMs??null});
      if(result?.outputDir){
        evidence.push({type:'qa-flow',flowId,path:result.outputDir});
        telemetry.push(...await readTelemetry(result.outputDir));
      }
    }catch(error){
      runs.push({flowId,status:'failed',error:error?.message||String(error),summary:error?.summary||null});
      checks.push({name:flowId,category:categories[flowId]||'crosscut-flow',status:'failed',critical:criticalFlows.has(flowId),error:error?.message||String(error)});
    }
  }
  ...
  const renderer=await runRendererHealthContract({
    sweepResult:rendererSweepFromTelemetry(telemetry),
    policy:crosscut.rendererHealthPolicy||{},
  });
  checks.push(...safeArray(renderer.checks));
  const findings=safeArray(renderer.findings);
  const consoleErrors=safeArray(renderer.consoleErrors);
  const networkErrors=safeArray(renderer.networkErrors);
  const policy=crosscut.policy||{};
  const summary=buildProductQaSummary({
    systemId:manifest.systemId,
    profile:'crosscut',
    checks,
    coverage:moduleResult.coverage,
    consoleErrors,
    networkErrors,
    findings,
    evidence,
  ...
  let artifacts=null;
  if(writeArtifacts){
    artifacts=await writeProductQaBundle({outputRoot,summary,coverage:moduleResult.coverage,consoleErrors,networkErrors,findings,evidence});
  }
...
export async function runCrosscutProfile(options={}){
  ...
  if(!result.productGate.allowed){
    const error=new Error(`QA crosscut gate failed for ${options.manifest?.systemId||'system'}`);
    error.crosscut=result;
    throw error;
  }
  return result;
}
