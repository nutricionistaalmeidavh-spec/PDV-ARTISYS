...
function sampleSale(overrides = {}) {
  return {
    id:'sale-1', saleNumber:'V-1001', status:'COMPLETED', subtotalCents:1500, discountCents:101, totalCents:1399, changeCents:0,
...
function sampleFiscalContext(overrides = {}) {
  return {
    ...
    series:'1', number:'42', operationNature:'VENDA',
    items:{
...
test('allocates sale discount deterministically in cents and preserves exact total', () => {
  const allocations = allocateDiscountCents([{ totalCents:1000 }, { totalCents:500 }], 101);
...
test('builds canonical NFC-e from completed sale without recalculating the sale total', () => {
  const input = { sale:sampleSale(), fiscalContext:sampleFiscalContext(), documentType:'nfce', environment:'homologation', reference:'V-1001' };
  const document = buildFiscalDocument(input);
  const repeated = buildFiscalDocument(input);
  ...
  assert.match(document.identification.numericCode, /^\d{8}$/);
  assert.equal(repeated.identification.numericCode, document.identification.numericCode);
  assert.notEqual(document.identification.numericCode, '00000042');
...
test('fails closed when a product has no resolved fiscal data and leaves the sale object untouched', () => {
  ...
  assert.throws(() => buildFiscalDocument({ sale, fiscalContext:context, documentType:'nfce', environment:'homologation', reference:'V-1001' }), /Dados fiscais ausentes.*p2/);
  assert.equal(JSON.stringify(sale), snapshot);
});

test('rejects non-completed sale and inconsistent canonical totals', () => {
  assert.throws(() => buildFiscalDocument({ sale:sampleSale({ status:'OPEN' }), fiscalContext:sampleFiscalContext(), documentType:'nfce', environment:'homologation', reference:'V-1001' }), /venda concluida/i);
  assert.throws(() => buildFiscalDocument({ sale:sampleSale({ totalCents:1400 }), fiscalContext:sampleFiscalContext(), documentType:'nfce', environment:'homologation', reference:'V-1001' }), /Total canonico inconsistente/);
});
