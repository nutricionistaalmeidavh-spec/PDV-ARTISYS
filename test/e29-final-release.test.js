...
test('package keeps E29 Windows NSIS and XLSX support in modular release 1.2.0', () => {
  const pkg = JSON.parse(read('package.json'));
...
test('release manifest remains deterministic and hashes supplied v1.2 artifacts', () => {
  ...
  try {
    const artifact = path.join(dir, 'ArtiSys-PDV-1.2.0-x64-Setup.exe');
...
test('all operations manuals and release metadata exist without future-delivery placeholders', () => {
  ...
  assert.match(readme, /ArtiSys PDV 1\.(0|1|2)/);
