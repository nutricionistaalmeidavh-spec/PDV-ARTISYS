...
function createLocalServer({runtime,host='127.0.0.1',port=4174,token='',bodyLimitBytes=1024*1024,allowedOrigins=[],requireTerminalAuth=false}={}){
  ...
  async function route(req,res){let handled=await selfServiceHandler(req,res);if(!handled)handled=await restaurantHandler(req,res);if(!handled)handled=await finalVerticalHandler(req,res);if(!handled)handled=await kitComboHandler(req,res);if(!handled)handled=await productVariantHandler(req,res);if(!handled)handled=await verticalHandler(req,res);if(!handled)handled=await enterpriseDepthHandler(req,res);if(!handled)handled=await fiscalBlock6Handler(req,res);if(!handled)handled=await fiscalBlock5Handler(req,res);if(!handled)await handler(req,res);}
