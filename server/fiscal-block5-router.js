...
const {queueNfeDanfe}=require('../js/domains/fiscal/nfe-printing');
...
function createFiscalBlock5Router({runtime,sessionStore,bodyLimitBytes=1024*1024}={}){
  ...
  return async function route(req,res){
    ...
    try{
      ...
      match=pathname.match(/^\/api\/v1\/fiscal\/documents\/([^/]+)\/danfe$/);if(method==='POST'&&match){const id=decodeURIComponent(match[1]);const body=await readJson(req,bodyLimitBytes);const document=runtime.fiscal.getDocument(id);if(!document)throw Object.assign(new Error('Documento fiscal nao encontrado.'),{statusCode:404});const job=document.documentType==='nfe'?queueNfeDanfe({runtime,documentId:id,actor:currentActor}):runtime.fiscal.queueDanfe(id,{actor:currentActor,width:body.width||42});sendJson(res,201,job);return true;}
