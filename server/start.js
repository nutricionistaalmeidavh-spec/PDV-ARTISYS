...
function resolveServerConfig(env = process.env, cwd = process.cwd()) {
  ...
  const allowedOrigins = String(env.PDV_ALLOWED_ORIGINS || '')
    .split(',')
    .map((value) => value.trim())
    .filter(Boolean);

  if (!Number.isInteger(port) || port < 1 || port > 65535) {
    throw new Error('PDV_PORT deve ser um inteiro entre 1 e 65535.');
  }

  const isLoopback = host === '127.0.0.1' || host === 'localhost' || host === '::1';
  if (!isLoopback && !token) {
    throw new Error('PDV_INSTALL_TOKEN e obrigatorio quando o servidor aceita conexoes LAN.');
  }

  return { host, port, token, dbPath, allowedOrigins };
}

function createServerFromEnvironment(env = process.env, cwd = process.cwd()) {
  const config = resolveServerConfig(env, cwd);
  const runtime = createPdvRuntime({ dbPath: config.dbPath });
...
async function main() {
  const app = createServerFromEnvironment();
  const address = await app.start();
  console.log(`PDV ArtiSys Local Server: http://${address.host}:${address.port}/api/v1/health`);
