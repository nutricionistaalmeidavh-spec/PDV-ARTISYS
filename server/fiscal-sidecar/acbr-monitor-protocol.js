...
function assertLoopbackHost(host) {
  ...
  if (!LOOPBACK_HOSTS.has(value)) throw new Error('ACBrMonitor deve ser acessado somente por loopback local.');
  return value;
}

function assertPort(value) {
  const port = Number(value);
  if (!Number.isInteger(port) || port < 1 || port > 65535) throw new Error('Porta ACBrMonitor invalida.');
  return port;
}
...
function renderNfceIni(document = {}) {
  if (document.documentType !== 'nfce') throw new Error('Renderer ACBr do Bloco 3 aceita somente NFC-e.');
  ...
  pushSection(lines, 'Emitente', [
    ['CNPJCPF', issuer.cnpj],
    ['xNome', issuer.legalName],
...
function parseAcbrResponse(raw, { expectedNumber = null, expectedSeries = null } = {}) {
  ...
  const fiscalSectionName = Object.keys(sections).find(name => /^NFE\d*$/i.test(name) && readKey(sections[name], 'CStat'))
    || Object.keys(sections).find(name => readKey(sections[name], 'CStat'));
  const fiscal = sections[fiscalSectionName] || {};
  const fileSectionName = Object.keys(sections).find(name => /^NFE_ARQ/i.test(name));
  const fileSection = sections[fileSectionName] || {};
  const cStatRaw = readKey(fiscal, 'CStat');
  const cStat = cStatRaw == null ? null : Number(cStatRaw);
  const xMotivo = readKey(fiscal, 'XMotivo') || readKey(fiscal, 'Msg') || null;
  const data = {
    cStat:Number.isFinite(cStat) ? cStat : null,
    xMotivo,
    chave:readKey(fiscal, 'ChDFe') || readKey(fiscal, 'ChNFe') || null,
    protocolo:readKey(fiscal, 'NProt') || null,
    receivedAt:readKey(fiscal, 'DhRecbto') || null,
    xmlPath:readKey(fileSection, 'Arquivo') || null,
    numero:expectedNumber == null ? null : String(expectedNumber),
    serie:expectedSeries == null ? null : String(expectedSeries)
  };
...
function createAcbrMonitorTcpTransport({
  ...
  const safeMax = Math.max(1024, Number(maxResponseBytes) || 2 * 1024 * 1024);
  if (typeof connect !== 'function') throw new TypeError('connect deve ser funcao.');

  async function send(command) {
    const text = String(command || '').trim();
    if (!text) throw new Error('Comando ACBrMonitor obrigatorio.');
    return new Promise((resolve, reject) => {
      const socket = connect({ host:safeHost, port:safePort });
      ...
      socket.on?.('data', chunk => {
        ...
        if (buffer.length > safeMax) return finish(new Error('Resposta ACBrMonitor excede o limite permitido.'));
      ...
      socket.once?.('close', () => {
        ...
        else if (!settled) finish(new Error('Conexao ACBrMonitor encerrada sem resposta.'));
      });
    });
  }
