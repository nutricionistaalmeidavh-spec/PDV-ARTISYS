...
async function readJson(req, maxBytes) {
  ...
  for await (const chunk of req) {total += chunk.length;if (total > maxBytes) {const error = new Error('Payload fiscal local excede o limite permitido.');error.statusCode = 413;throw error;}chunks.push(chunk);}
  if (!chunks.length) return {};
  const raw = Buffer.concat(chunks).toString('utf8');if (!raw.trim()) return {};
  try { return JSON.parse(raw); } catch { const error = new Error('JSON fiscal local invalido.');error.statusCode = 400;throw error; }
}

function validateType(value) {
  const type = String(value || '').trim().toLowerCase();
  if (!DOCUMENT_TYPES.has(type)) {const error = new Error('Tipo de documento fiscal invalido.');error.statusCode = 400;throw error;}
  return type;
}

function validateReference(value) {
  const reference = String(value || '').trim();
  if (!reference || reference.length > 80 || !/^[A-Za-z0-9._-]+$/.test(reference)) {const error = new Error('Referencia fiscal invalida.');error.statusCode = 400;throw error;}
  return reference;
}
...
function normalizeAdapterResult(result) {
  ...
  return {
    ok:Boolean(result.ok),status:Number(result.status || (result.ok ? 200 : 500)),data:result.data ?? null,error:result.error ? String(result.error) : null,
...
function createFiscalSidecar({ adapter, host = '127.0.0.1', port = 0, maxBodyBytes = 1024 * 1024 } = {}) {
  ...
  for (const method of ['status','issue','query','cancel']) if (typeof adapter[method] !== 'function') throw new TypeError(`Fiscal adapter must implement ${method}().`);
  ...
  async function handle(req, res) {
    ...
    if (method === 'GET' && url.pathname === '/v1/health') {
      ...
      return sendJson(res, 200, {healthy:true,service:'artisys-fiscal-sidecar',loopbackOnly:true,adapter:{ok:adapterStatus.ok,status:adapterStatus.status,data:adapterStatus.data,error:adapterStatus.error}});
    }
    ...
    if (isCancel) {
      ...
      const result = await adapter.cancel({type,reference,justification:String(body.justification || ''),environment:String(body.environment || 'homologation')});
      return sendJson(res, 200, { result:normalizeAdapterResult(result) });
    }

    if (method === 'POST') {
      const body = await readJson(req, maxBodyBytes);
      const result = await adapter.issue({type,reference,payload:body.payload && typeof body.payload === 'object' ? body.payload : {},environment:String(body.environment || 'homologation')});
      return sendJson(res, 200, { result:normalizeAdapterResult(result) });
    }

    if (method === 'GET') {
      const result = await adapter.query({
        type,reference,environment:String(url.searchParams.get('environment') || 'homologation'),
        accessKey:String(url.searchParams.get('accessKey') || '') || null
      });
      return sendJson(res, 200, { result:normalizeAdapterResult(result) });
    }
  ...
  function start() {
    ...
    server = http.createServer((req, res) => {Promise.resolve(handle(req, res)).catch(error => {const statusCode = Number(error?.statusCode || 500);sendJson(res, statusCode >= 400 && statusCode <= 599 ? statusCode : 500, {error:error?.message || String(error)});});});
    server.keepAliveTimeout = 5000;
    return new Promise((resolve, reject) => {
      const onError = error => {server?.removeListener('listening', onListening);server = null;reject(error);};
      const onListening = () => {server?.removeListener('error', onError);resolve(server.address());};
      server.once('error', onError);server.once('listening', onListening);server.listen(safePort, safeHost);
    });
  }
