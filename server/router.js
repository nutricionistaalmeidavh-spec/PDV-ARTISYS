  ...
  return async function route(request, response) {
    try {
      ...
      if (request.method === 'GET' && pathname === '/api/v1/setup/status') {
        sendJson(response, 200, { needsSetup: runtime.catalog.countUsers() === 0 }, request, allowedOrigins); return;
      }
      if (request.method === 'POST' && pathname === '/api/v1/setup/admin') {
        checkInstallToken(request);
        if (runtime.catalog.countUsers() !== 0) throw new HttpError(409, 'Configuracao inicial ja concluida.');
        ...
        sendJson(response, 201, user, request, allowedOrigins); return;
      }
      ...
      if (/UNIQUE constraint failed/.test(error.message || '')) status = 409;
      sendJson(response, status, { error: error.message || 'Erro interno.' }, request, allowedOrigins);
    }
  };
}
