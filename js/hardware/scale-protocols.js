...
const SERIAL_9600_8N1 = Object.freeze({ baudRate:9600, dataBits:8, stopBits:1, parity:'none' });
...
function clonePreset(preset) {
  return { ...preset, models:[...preset.models], serial:{...preset.serial} };
}
...
function createScaleProtocol({ preset } = {}) {
  const registry = createScaleProtocolRegistry();
  const presetConfig = registry.getPreset(preset);
  const protocol = registry.getProtocolForPreset(preset);
  return Object.freeze({
    id:protocol.id,
    preset:presetConfig.id,
    manufacturer:presetConfig.manufacturer,
    models:[...presetConfig.models],
    documentationStatus:presetConfig.documentationStatus,
    serial:{...presetConfig.serial},
