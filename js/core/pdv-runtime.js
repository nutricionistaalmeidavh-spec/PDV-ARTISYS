...
const { runFinanceReceivableMigrations }=require('./database/finance-receivable-migrations');
...
function createPdvRuntime({
  ...
  runFinanceReceivableMigrations(db,now);
  ...
  registerFinanceEffects({bus,financeService:finance,saleService:sales,settingsService:settings,effectStore,db});
