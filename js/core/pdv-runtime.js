...
function createPdvRuntime({
  ...
  fiscalProviderResolver=async()=>null,nfseProviderResolver=async()=>null,fiscalAutoIssueResolver=null,fiscalArchiveDir=null,receiptOptions={},serverVersion='1.0.0',minimumTerminalVersion='1.0.0',capabilities,
  ...
  return {db,outbox,effectStore,bus,dispatcher,catalog,productPhotos,catalogCustomization,kitsCombos,inventory,logistics,procurement,orders,recipes,sales,commissions,cash,returns,finance,reports,printing,nonFiscalPrinting,fiscal,fiscalConfiguration,fiscalProduction,nfse,modules,onboarding,mobileAccess,hardwareCompatibility,restaurant,restaurantSettlement,kitchen,mobileDevices,restaurantReports,pizzeria,delivery,fastFood,marketBakery,retail,services,workshop,selfService,terminals,mutations,backups,settings,imports,logger,health,diagnostics,pilot,backupDir:resolvedBackupDir,diagnosticsDir:resolvedDiagnosticsDir,fiscalArchiveDir:resolvedFiscalArchiveDir,dispatchPending:()=>dispatcher.dispatchPending(),close(){db.close();}};
}
