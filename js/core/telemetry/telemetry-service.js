...
function createTelemetryService({db,settings,logger=null,identity,now=()=>new Date().toISOString(),idFactory=p=>`${p}-${Date.now()}`,httpSender=null,appVersion='0.0.0',releaseId='dev',schemaVersionResolver=()=>0,defaultEndpoint='',random=Math.random}={}){
 ...
 const endpoint=()=>String(settings.get('telemetry.endpoint',{scope:'global',defaultValue:defaultEndpoint})||'').trim();
 ...
 async function flush(){const summary={attempted:0,sent:0,retryable:0,discarded:0,paused:false};try{if(!enabled()||!endpoint()||!httpSender?.sendBatch){summary.paused=true;return summary;}const rows=queue.listReady(batchSize());if(!rows.length)return summary;summary.attempted=rows.length;let response;try{response=await httpSender.sendBatch({schema_version:1,events:rows.map(row=>row.payload)});}catch{queue.reschedule(rows.map(r=>r.id),retryAt(Math.max(...rows.map(r=>r.attempts),0)));summary.retryable=rows.length;return summary;}const status=Number(response?.status||0);const ids=rows.map(r=>r.id);if(status>=200&&status<300){queue.ack(ids);summary.sent=rows.length;}else if([400,413,422].includes(status)){queue.discard(ids);summary.discarded=rows.length;}else if([401,403].includes(status)){summary.paused=true;}else{queue.reschedule(ids,retryAt(Math.max(...rows.map(r=>r.attempts),0)));summary.retryable=rows.length;}return summary;}catch(error){safeLog(error?.message);summary.paused=true;return summary;}}
 function status(){let pending=0;try{pending=queue.count();}catch{}return{enabled:enabled(),endpointConfigured:Boolean(endpoint()),pending};}
 function setEnabled(value,actor={}){return settings.set('telemetry.enabled',Boolean(value),{scope:'global',actor});}
 function close(){return true;}
 return{record,flush,status,setEnabled,close,queue};
}
