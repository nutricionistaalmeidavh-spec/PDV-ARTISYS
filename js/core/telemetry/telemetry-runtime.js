...
function attachRuntimeTelemetry({runtime,httpSender=null,defaultEndpoint='',releaseId='',appVersion='',now=()=>new Date().toISOString(),idFactory}={}){
 ...
 Object.defineProperty(runtime,'telemetry',{value:telemetry,writable:false,enumerable:true,configurable:false});lastTelemetry=telemetry;for(const handler of Array.from(ATTACH_LISTENERS)){try{handler(telemetry);}catch{}}
 return telemetry;
}
