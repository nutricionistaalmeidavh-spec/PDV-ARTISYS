...
const STATUSES=new Set(['VERIFIED','PARTIAL','UNSUPPORTED','BLOCKED_EXTERNAL']);
...
function createHardwareCompatibilityService({db,now=()=>new Date().toISOString(),idFactory=p=>`${p}-${randomUUID()}`}={}){
  ...
  function listEvidence({status=null,kind=null}={}){const clauses=[];const params=[];if(status){const value=String(status).toUpperCase();if(!STATUSES.has(value))throw new Error('Status de homologacao invalido.');clauses.push('status=?');params.push(value);}if(kind){const value=String(kind).toUpperCase();if(!KINDS.has(value))throw new Error('Tipo de periferico invalido.');clauses.push('kind=?');params.push(value);}return db.prepare(`SELECT * FROM hardware_compatibility_evidence${clauses.length?` WHERE ${clauses.join(' AND ')}`:''} ORDER BY manufacturer,model,tested_at DESC,id`).all(...params).map(map);}
  return{recordEvidence,listEvidence,STATUSES,KINDS};
}
module.exports={createHardwareCompatibilityService,STATUSES,KINDS};
