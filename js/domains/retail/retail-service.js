...
function createRetailService({db,modules,sales,now=()=>new Date().toISOString(),idFactory=p=>`${p}-${randomUUID()}`}={}){
  ...
  function mapVariant(row){
    ...
    return{variantId:row.id,productId:row.product_id,productName:row.product_name,name:row.name,sku:row.sku,barcode:row.barcode,attributes:parseJson(row.attributes_json,{}),priceDeltaCents:row.price_delta_cents,costCents:row.cost_cents,unitPriceCents:Number(row.base_price_cents)+Number(row.price_delta_cents),quantity:Number(balance.quantity||0),minimumStock:Number(balance.minimum_stock||0),parentTracksStock:Boolean(row.parent_track_stock)};
  }
  ...
  function setVariantStock(variantId,quantity,actor={}){
    gate();const row=variantRow(variantId);if(row.parent_track_stock)throw new Error('Produto com estoque por variante deve manter o estoque do produto pai desativado.');const next=roundQuantity(Number(quantity));if(!Number.isFinite(next)||next<0)throw new Error('Saldo da variacao invalido.');
