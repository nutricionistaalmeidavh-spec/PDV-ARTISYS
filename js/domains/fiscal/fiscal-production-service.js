...
const MANUAL_EVIDENCE_KEYS = new Set(['csc','sidecar','sefaz']);
...
function createFiscalProductionService({
  ...
  // Crash no meio do envio nunca vira retry cego: reinicia em reconciliacao obrigatoria.
  ...
  function objectiveHomologation(){
    ...
    const danfe=Number(db.prepare("SELECT COUNT(*) AS count FROM fiscal_documents WHERE environment='homologation' AND danfe_print_job_id IS NOT NULL").get()?.count||0)>0;
    return {authorized,cancelled,danfe};
  }

  function getReadiness(){
    const settings=fiscalConfiguration.getCompanySettings();const evidence=evidenceMap();const checks=[];
    ...
    push('company_cnpj','CNPJ',Boolean(settings?.cnpj),settings?.cnpj?'CNPJ fiscal configurado.':'CNPJ fiscal ausente.');
    push('company_ie','Inscricao estadual',Boolean(settings?.stateRegistration),settings?.stateRegistration?'IE configurada.':'Inscricao estadual ausente.');
    push('company_crt','CRT/regime',Boolean(settings?.crt),settings?.crt?'CRT configurado.':'CRT/regime ausente.');
    const addressReady=Boolean(settings?.address?.street&&settings?.address?.number&&settings?.address?.district&&settings?.address?.cityCode&&settings?.address?.city&&settings?.address?.state&&settings?.address?.zip);
    push('company_address','Endereco fiscal',addressReady,addressReady?'Endereco fiscal completo.':'Endereco fiscal incompleto.');
    push('series','Serie NFC-e',Boolean(settings?.series),settings?.series?`Serie ${settings.series} configurada.`:'Serie fiscal ausente.');
    ...
    const certificate=fiscalConfiguration.getCertificateMetadata();const validTo=certificate?.validTo?Date.parse(certificate.validTo):NaN;const nowMs=Date.parse(now());
    const certificateObjective=Boolean(certificate&&Number.isFinite(validTo)&&Number.isFinite(nowMs)&&validTo>nowMs);
    const hom=objectiveHomologation();const backupStatus=backups?.getBackupStatus?.()||null;const backupObjective=Boolean(backupStatus?.latest?.valid);
    for(const [key,label] of EXTERNAL_CHECKS){
      const row=evidence.get(key);let passed=false;let message='Evidencia obrigatoria ainda nao confirmada.';let source='missing';let checkedAt=row?.checkedAt||null;
      if(key==='certificate'){passed=certificateObjective;message=passed?`Certificado valido ate ${certificate.validTo}.`:'Certificado A1 valido e com validade verificada e obrigatorio.';source='computed';}
      else if(key==='homologation_authorized'){passed=hom.authorized;message=passed?'Ha NFC-e autorizada em homologacao.':'Falta NFC-e realmente autorizada em homologacao.';source='computed';}
      else if(key==='homologation_cancelled'){passed=hom.cancelled;message=passed?'Ha cancelamento confirmado em homologacao.':'Falta cancelamento realmente confirmado em homologacao.';source='computed';}
      else if(key==='danfe_print'){passed=hom.danfe;message=passed?'Ha DANFE de homologacao enfileirado para impressao.':'Falta evidencia objetiva de DANFE em homologacao.';source='computed';}
      else if(key==='backup'){passed=backupObjective;message=passed?`Backup valido: ${backupStatus.latest.id}.`:'Crie e valide um backup antes de ativar producao.';source='computed';checkedAt=backupStatus?.latest?.validatedAt||null;}
      else {passed=Boolean(row?.passed);message=row?.message||(key==='csc'?'Importe o CSC no cofre local.':'Execute o teste operacional correspondente.');source=row?'evidence':'missing';}
      push(key,label,passed,message,source,checkedAt);
    }
  ...
  function recordEvidence(key,input={},actor={}){
    requireManager(actor);const safeKey=String(key||'').trim();if(!MANUAL_EVIDENCE_KEYS.has(safeKey))throw new Error('Este item do checklist exige evidencia objetiva do sistema e nao pode ser liberado manualmente.');
  ...
  function enterContingency(documentId,{reason,actor={}}={}){
    ...
    withTransaction(db,()=>{
      db.prepare(`INSERT INTO fiscal_contingency(fiscal_document_id,status,reason,entered_at,tp_emis,document_json,generated_xml,access_key,transmission_attempts,updated_at) VALUES(?,'CONTINGENCY_PENDING',?,?, '9',?,NULL,NULL,0,?)`)
      ...
      recordFiscalEvent(documentId,'CONTINGENCY_PENDING','PENDING',{reason:safeReason,tpEmis:'9'});
  ...
  async function prepareContingency(documentId,{actor={}}={}){
    requireManager(actor);const entry=getContingency(documentId,{includeXml:true});if(!entry)throw new Error('Contingencia fiscal nao encontrada.');
    if(entry.status==='ISSUED'&&entry.generatedXml)return getContingency(documentId);if(entry.status!=='CONTINGENCY_PENDING')throw new Error('Somente contingencia pendente pode gerar e assinar o XML offline.');
    ...
    const timestamp=now();db.prepare("UPDATE fiscal_contingency SET status='ISSUED',generated_xml=?,access_key=?,updated_at=?,last_error=NULL WHERE fiscal_document_id=?").run(xml,key,timestamp,String(documentId));
    recordFiscalEvent(documentId,'CONTINGENCY_ISSUED','PENDING',{accessKey:key});
    if(printing){
  ...
  async function transmitContingency(documentId,{actor={}}={}){
    ...
    else{fiscal.markUnknown(documentId,{...result,indeterminate:true},actor);db.prepare("UPDATE fiscal_contingency SET status='RECONCILING',last_error=?,updated_at=? WHERE fiscal_document_id=?").run(String(result?.error||'Resultado de transmissao inconclusivo.'),now(),String(documentId));recordFiscalEvent(documentId,'CONTINGENCY_UNKNOWN','UNKNOWN',{});}
...
module.exports={EXTERNAL_CHECKS,MANUAL_EVIDENCE_KEYS,createFiscalProductionService};