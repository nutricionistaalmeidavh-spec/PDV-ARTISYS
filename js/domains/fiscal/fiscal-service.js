...
const { assertFiscalTransition, FISCAL_STATES } = require('./fiscal-state-machine');
...
function normalizeCancelReason(value){const reason=String(value||'').trim().replace(/\s+/g,' ');if(reason.length<15||reason.length>255)throw new Error('Justificativa de cancelamento deve conter entre 15 e 255 caracteres.');return reason;}

function createFiscalService({ db, outbox, artifactStore=null, printing=null, now = () => new Date().toISOString(), idFactory = p => `${p}-${randomUUID()}` } = {}) {
  ...
  const hasBlock5=Boolean(db.prepare("SELECT 1 FROM pragma_table_info('fiscal_documents') WHERE name='cancellation_protocol'").get());
  ...
  function mapDocument(row) {
    ...
    return {
      ...
      cancellationProtocol:hasBlock5?row.cancellation_protocol:null,cancellationXmlPath:hasBlock5?row.cancellation_xml_path:null,
      cancellationReason:hasBlock5?row.cancellation_reason:null,danfePrintJobId:hasBlock5?row.danfe_print_job_id:null,
  ...
  function persistProviderXml(row,data,kind,timestamp){
    const sourcePath=data.xmlPath||data.xml_path||null;const xml=data.xml||data.xmlContent||null;
    if(!artifactStore)return sourcePath;
    return artifactStore.persistXml({kind,document:{id:row.id,reference:row.reference},xml,sourcePath,occurredAt:timestamp});
  }
  ...
  function retryIssue(id,{actor={},mutationId=null}={}){
    return withTransaction(db,()=>{
      ...
      if(hasLifecycle)assertFiscalTransition(state,'PENDING');const timestamp=now();
  ...
  function markProcessing(id,actor={}){
    return withTransaction(db,()=>{
      ...
      recordEvent(id,{eventType:'PROCESSING',status:'PROCESSING'});writeAudit(db,{action:'fiscal.state.processing',entity:'fiscal-document',entityId:String(id),actor,context:{reference:row.reference}},now);return getDocument(id);
    });
  }
  ...
  function markAuthorized(id,result={},actor={}){
    return withTransaction(db,()=>{
      ...
      const durableXmlPath=persistProviderXml(row,data,'authorized',timestamp)||data.xmlPath||data.xml_path||null;
      if(hasLifecycle)db.prepare(`UPDATE fiscal_documents SET status='ISSUED',lifecycle_status='AUTHORIZED',access_key=?,number=?,series=?,issued_at=?,authorized_at=?,authorization_protocol=?,xml_path=COALESCE(?,xml_path),sefaz_code=?,sefaz_message=?,reconcile_required=0,last_reconciled_at=CASE WHEN ?='UNKNOWN' THEN ? ELSE last_reconciled_at END,last_reconcile_status=CASE WHEN ?='UNKNOWN' THEN 'AUTHORIZED' ELSE last_reconcile_status END,last_error=NULL,last_transition_at=?,updated_at=? WHERE id=?`)
        .run(key,data.numero!=null?String(data.numero):row.number,data.serie!=null?String(data.serie):row.series,timestamp,timestamp,protocol,durableXmlPath,sefazCode(result),sefazMessage(result),state,timestamp,state,timestamp,timestamp,String(id));
      ...
      recordEvent(id,{eventType:'AUTHORIZED',status:'AUTHORIZED',result,metadata:{xmlArchived:Boolean(durableXmlPath)}});
      outbox.insert(eventEnvelope({type:'fiscal.issued',documentId:String(id),actor,payload:{saleId:row.sale_id,reference:row.reference,accessKey:key}}));return getDocument(id);
    });
  }
  ...
  function markRejected(id,result={},actor={}){
    return withTransaction(db,()=>{
      ...
      recordEvent(id,{eventType:'REJECTED',status:'REJECTED',result});outbox.insert(eventEnvelope({type:'fiscal.rejected',documentId:String(id),actor,payload:{saleId:row.sale_id,reference:row.reference,sefazCode:sefazCode(result),message}}));return getDocument(id);
    });
  }
  ...
  function markUnknown(id,result={},actor={}){
    return withTransaction(db,()=>{
      ...
      recordEvent(id,{eventType:'UNKNOWN',status:'UNKNOWN',result});outbox.insert(eventEnvelope({type:'fiscal.unknown',documentId:String(id),actor,payload:{saleId:row.sale_id,reference:row.reference,error:message}}));return getDocument(id);
    });
  }
  ...
  function markFailed(id,result={},actor={}){
    return withTransaction(db,()=>{
      ...
      recordEvent(id,{eventType:'FAILED',status:'FAILED',result});outbox.insert(eventEnvelope({type:'fiscal.failed',documentId:String(id),actor,payload:{saleId:row.sale_id,reference:row.reference,error:message}}));return getDocument(id);
    });
  }
  ...
  function requestReconcile(id,{actor={},mutationId=null}={}){
    return withTransaction(db,()=>{
      ...
      recordEvent(id,{eventType:'RECONCILE_REQUESTED',status:'UNKNOWN'});outbox.insert(eventEnvelope({type:'fiscal.reconcile-requested',documentId:String(id),actor,mutationId,payload:{saleId:row.sale_id,provider:row.provider,documentType:row.document_type,environment:row.environment,reference:row.reference}}));writeAudit(db,{action:'fiscal.reconcile.request',entity:'fiscal-document',entityId:String(id),actor,context:{reference:row.reference}},now);return getDocument(id);
    });
  }

  function markReconcileNotFound(id,result={},actor={}){
    return withTransaction(db,()=>{const row=requireDocument(id);if(currentState(row)!=='UNKNOWN')throw new Error('Reconciliacao NOT_FOUND exige documento UNKNOWN.');saveProviderResponse(id,{status:result.status,data:result.data||null,error:result.error||null});const timestamp=now();if(hasLifecycle)db.prepare("UPDATE fiscal_documents SET reconcile_required=0,last_reconciled_at=?,last_reconcile_status='NOT_FOUND',sefaz_code=?,sefaz_message=?,last_transition_at=?,updated_at=? WHERE id=?").run(timestamp,sefazCode(result),sefazMessage(result),timestamp,timestamp,String(id));recordEvent(id,{eventType:'RECONCILED_NOT_FOUND',status:'UNKNOWN',result});writeAudit(db,{action:'fiscal.reconcile.not-found',entity:'fiscal-document',entityId:String(id),actor,context:{reference:row.reference}},now);return getDocument(id);});
  }
  function markReconcileUnknown(id,result={},actor={}){
    return withTransaction(db,()=>{const row=requireDocument(id);if(currentState(row)!=='UNKNOWN')throw new Error('Reconciliacao inconclusiva exige documento UNKNOWN.');const timestamp=now();saveProviderResponse(id,{status:result.status,data:result.data||null,error:result.error||null});if(hasLifecycle)db.prepare("UPDATE fiscal_documents SET reconcile_required=1,last_reconciled_at=?,last_reconcile_status='UNKNOWN',sefaz_code=?,sefaz_message=?,updated_at=? WHERE id=?").run(timestamp,sefazCode(result),sefazMessage(result),timestamp,String(id));recordEvent(id,{eventType:'RECONCILED_UNKNOWN',status:'UNKNOWN',result});return getDocument(id);});
  }

  function requestCancel(id,{reason,actor={},mutationId=null}={}){
    requireManager(actor);const justification=normalizeCancelReason(reason);
    return withTransaction(db,()=>{
      const row=requireDocument(id);const state=currentState(row);if(state==='CANCELLED')throw new Error('Documento fiscal ja cancelado.');if(state!=='AUTHORIZED')throw new Error('Somente documento fiscal autorizado pode ser cancelado.');if(!row.access_key)throw new Error('Documento autorizado sem chave de acesso; cancelamento bloqueado.');
      if(hasBlock5)db.prepare('UPDATE fiscal_documents SET cancellation_reason=?,updated_at=? WHERE id=?').run(justification,now(),String(id));
      recordEvent(id,{eventType:'CANCEL_REQUESTED',status:'AUTHORIZED',metadata:{reason:justification}});
      outbox.insert(eventEnvelope({type:'fiscal.cancel-requested',documentId:String(id),actor,mutationId,payload:{saleId:row.sale_id,provider:row.provider,documentType:row.document_type,environment:row.environment,reference:row.reference,reason:justification,accessKey:row.access_key}}));
      writeAudit(db,{action:'fiscal.cancel.request',entity:'fiscal-document',entityId:String(id),actor,context:{reference:row.reference,reason:justification}},now);return getDocument(id);
    });
  }

  function markCancelFailed(id,result={},actor={}){
    return withTransaction(db,()=>{
      const row=requireDocument(id);if(currentState(row)!=='AUTHORIZED')return getDocument(id);const message=sefazMessage(result)||'Falha no cancelamento fiscal.';const timestamp=now();
      db.prepare('UPDATE fiscal_documents SET last_error=?,updated_at=? WHERE id=?').run(message,timestamp,String(id));recordEvent(id,{eventType:result.indeterminate?'CANCEL_UNKNOWN':'CANCEL_FAILED',status:'AUTHORIZED',result,metadata:{operation:'cancel'}});writeAudit(db,{action:'fiscal.cancel.failed',entity:'fiscal-document',entityId:String(id),actor,context:{reference:row.reference,sefazCode:sefazCode(result),message}},now);return getDocument(id);
    });
  }

  function markCancelled(id,result={},actor={},reason=null){
    return withTransaction(db,()=>{
      const row=requireDocument(id);const state=currentState(row);if(state==='CANCELLED')return getDocument(id);if(hasLifecycle)assertFiscalTransition(state,'CANCELLED');const data=pickData(result);const timestamp=now();const protocol=data.protocolo||data.nProt||null;const storedXml=persistProviderXml(row,data,'cancellation',timestamp);const justification=reason||row.cancellation_reason||null;
      if(hasLifecycle&&hasBlock5)db.prepare("UPDATE fiscal_documents SET status='CANCELLED',lifecycle_status='CANCELLED',cancelled_at=?,reconcile_required=0,last_error=NULL,last_transition_at=?,cancellation_protocol=?,cancellation_xml_path=COALESCE(?,cancellation_xml_path),cancellation_reason=COALESCE(?,cancellation_reason),sefaz_code=?,sefaz_message=?,updated_at=? WHERE id=?").run(timestamp,timestamp,protocol,storedXml,justification,sefazCode(result),sefazMessage(result),timestamp,String(id));
      else if(hasLifecycle)db.prepare("UPDATE fiscal_documents SET status='CANCELLED',lifecycle_status='CANCELLED',cancelled_at=?,reconcile_required=0,last_error=NULL,last_transition_at=?,updated_at=? WHERE id=?").run(timestamp,timestamp,timestamp,String(id));
      ...
      recordEvent(id,{eventType:'CANCELLED',status:'CANCELLED',result,metadata:{reason:justification,xmlArchived:Boolean(storedXml)}});outbox.insert(eventEnvelope({type:'fiscal.cancelled',documentId:String(id),actor,payload:{saleId:row.sale_id,reference:row.reference}}));writeAudit(db,{action:'fiscal.cancelled',entity:'fiscal-document',entityId:String(id),actor,context:{reference:row.reference,protocol}},now);return getDocument(id);
    });
  }

  function listDocuments(filters={}){const clauses=[];const params=[];if(filters.saleId){clauses.push('sale_id=?');params.push(String(filters.saleId));}if(filters.status){if(hasLifecycle&&Object.values(FISCAL_STATES).includes(String(filters.status).toUpperCase())){clauses.push('lifecycle_status=?');params.push(String(filters.status).toUpperCase());}else{clauses.push('status=?');params.push(String(filters.status).toUpperCase());}}if(filters.documentType){clauses.push('document_type=?');params.push(String(filters.documentType).toLowerCase());}if(filters.environment){clauses.push('environment=?');params.push(String(filters.environment).toLowerCase());}return db.prepare(`SELECT * FROM fiscal_documents${clauses.length?` WHERE ${clauses.join(' AND ')}`:''} ORDER BY created_at DESC,id DESC`).all(...params).map(mapDocument);}
  ...
  function getMonitorDocument(id){const document=getDocument(id);if(!document)throw new Error('Documento fiscal nao encontrado.');const sale=db.prepare('SELECT id,sale_number,status,total_cents,completed_at FROM sales WHERE id=?').get(document.saleId);return{document,events:listEvents(id),sale:sale?{id:sale.id,saleNumber:sale.sale_number,status:sale.status,totalCents:sale.total_cents,completedAt:sale.completed_at}:null};}
  function readXml(id,kind='authorized'){const document=getDocument(id);if(!document)throw new Error('Documento fiscal nao encontrado.');const normalized=String(kind||'authorized').toLowerCase();if(!['authorized','cancellation'].includes(normalized))throw new Error('Tipo de XML fiscal invalido.');const filePath=normalized==='authorized'?document.xmlPath:document.cancellationXmlPath;if(!artifactStore)throw new Error('Arquivo fiscal local indisponivel.');return artifactStore.readXml(filePath);}
  function queueDanfe(id,{actor={},width=42}={}){if(!printing)throw new Error('Servico de impressao indisponivel para DANFE.');const document=getDocument(id);if(!document)throw new Error('Documento fiscal nao encontrado.');const text=renderDanfeNfce({document,width});const job=printing.queueJob({type:'DANFE_NFCE',entityType:'fiscal-document',entityId:document.id,payload:{text,width:Number(width)||42,documentType:'nfce',accessKey:document.accessKey},actor});if(hasBlock5)db.prepare('UPDATE fiscal_documents SET danfe_print_job_id=?,updated_at=? WHERE id=?').run(job.id,now(),String(id));recordEvent(id,{eventType:'DANFE_QUEUED',status:document.lifecycleStatus,metadata:{printJobId:job.id,width:Number(width)||42}});writeAudit(db,{action:'fiscal.danfe.queue',entity:'fiscal-document',entityId:String(id),actor,context:{printJobId:job.id}},now);return job;}

  return {requestIssue,retryIssue,requestReconcile,requestCancel,getDocument,getByReference,listDocuments,listEvents,getMonitorDocument,readXml,queueDanfe,markProcessing,markAuthorized,markIssued:markAuthorized,markRejected,markUnknown,markFailed,markReconcileNotFound,markReconcileUnknown,markCancelFailed,markCancelled};
}
