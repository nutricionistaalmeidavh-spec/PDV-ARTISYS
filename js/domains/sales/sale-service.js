...
function createSaleService({db,outbox,now=()=>new Date().toISOString(),idFactory=p=>`${p}-${randomUUID()}`}={}){
  ...
  function completeSale(saleId,{payments=[],actor={},mutationId=null}={}){
    return withTransaction(db,()=>{
      ...
      const creditMethods=['STORE_CREDIT'];
      const paymentSummary=resolvePayment({totalCents:totals.totalCents,payments:normalizedPayments,creditMethods,changeMethods:['CASH'],availableCreditCents:customer?Math.max(customer.credit_limit_cents-customer.credit_used_cents,0):0,customerName:customer?.name});
  ...
  return {openSale,addItem,updateItemQuantity,removeItem,applyDiscount,suspendSale,resumeSale,completeSale,cancelSale,getSale};
}
